  ...
  const projects = [
    {
      ...
      name: "Coffee Place",
      description:
        "Embarking on the day with the rich, inviting aroma of coffee is a truly wise choice. Imagine an app designed to unveil the delightful world of coffee varieties, guiding you through their nuances and helping you effortlessly place your perfect order. Your journey into the realm of coffee has never been this delightful—discover, savor, and indulge with every sip.",
      tech: ["React Native", "TypeScript"],
      live: "",
      github: "https://github.com/yashodhicy/coffee",
    },
    {
      image: "/house.png",
      name: "RentHub",
    ...
    <div className="h-full">
      ...
      <div className="flex justify-center w-88">
        ...
        <div
          ...
          className="grid grid-flow-row gap-4 mt-4 mx-12 snap-x snap-mandatory w-full overflow-y-scroll scroll-smooth scrollbar-hide sm:grid-flow-col sm:my-10"
        >
          {projects.map((p, index) => (
            <div
              key={index}
              className="flex flex-col shrink ring-4 rounded-xl border-2 border-white mx-4 my-6 py-2 sm:h-96 sm:w-72  text-white hover:scale-110  snap-normal snap-center  "
            >
              <h2 className=" text-lg text-center w-full">{p.name}</h2>
              <img
                className="flex shrink mx-3 h-1/2 md:mb-5 rounded-lg"
                src={p.image}
              ></img>
              ...
              <div className="grid grid-flow-row popup-content m-3 sm:grid-flow-col">
                ...
                <div className="flex flex-col box-border overflow-y-scroll">
                  ...
                  <span className=" box-content flex justify-between mt-5 gap-3 sm:flex-row sm:gap-4 ">
                    <Link
                      target="_blank"
                      href={projects[selectedProjectIndex].github}
                      className=" ring-2 rounded-lg border border-white px-2 hover:bg-white hover:text-sky-950 hover:font-bold"
                    >
                      {" "}
                      Github code{" "}
                    </Link>
                    {projects[selectedProjectIndex].live === "" ? (
                      <p>Live link is coming soon.</p>
                    ) : (
                      <Link
                        target="_blank"
                        href={projects[selectedProjectIndex].live}
                        className=" ring-2 rounded-lg border border-white px-2 hover:bg-white hover:text-sky-950 hover:font-bold"
                      >
                        {" "}
                        Live Link{" "}
